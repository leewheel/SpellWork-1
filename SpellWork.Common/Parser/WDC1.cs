...
namespace SpellWork.Parser
{
	public class WDC1 : WDB6
	{
        ...
        public void LoadDefinitionSizes<T>()
        {
            //if (HasOffsetTable)
            //    return;

            columnSizes = Parser.GetClassSize<T>();
            columnSizesReal = Parser.GetClassSize<T>(true, HasIndexTable);
            ...
            {
                bool hasRelationShipColumnDefined = false;
                foreach (FieldInfo field in typeof(T).GetFields())
                {
                    if (field.GetCustomAttribute(typeof(RelationFieldAttribute)) != null)
                    {
                        if (hasRelationShipColumnDefined)
                            throw new Exception($"Struct " + typeof(T) + " has multiple RelationShip fields defined");
                        hasRelationShipColumnDefined = true;
                    }
                }
                if (!hasRelationShipColumnDefined)
                    throw new Exception($"Struct " + typeof(T) + " has no RelationShip field defined");
            }
        }
        public new Dictionary<int, byte[]> ReadOffsetData(BinaryReader dbReader, long pos)
		{
            ...
            for (int i = 0; i < ColumnMeta.Count; i++)
			{
				...
				{
                    int byteCount = columnSizesReal[i];
                    
                    int elements = (int)ColumnMeta[i].AdditionalDataSize / byteCount;
					...
					for (int j = 0; j < elements / cardinality; j++)
						ColumnMeta[i].PalletValues.Add(dbReader.ReadBytes(cardinality * byteCount));
				}
			}
				...
				else
				{
					...
					if (RelationShipData != null)
					{
						...
						if (RelationShipData.Entries.TryGetValue((uint)i, out byte[] foreignData))
                            ...
                            throw new Exception($"Missing relationship data");
